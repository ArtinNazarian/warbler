...
def update_password():
    ...
    if form.validate_on_submit():
        if User.authenticate(current_user.username, form.old_password.data):          
            password1 = form.new_password_1.data
            password2 = form.new_password_2.data
            if password1 == password2:  
                hashed_password = User.reset(password1)
                current_user.password = hashed_password
                db.session.commit()
                flash("Password updated successfully")
                return redirect(f'/users/{current_user.id}')
            else:
                flash("New password doesn't match. Try Again!")
                return render_template('users/password.html', form=form, user_id=current_user.id)
        else:
            flash("Wrong password. Try again")   
    return render_template('users/password.html', form=form, user_id=current_user.id)
...
def like_warbler(message_id):
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect('/')
    
    liked_message = Message.query.get_or_404(message_id)
    print(f"++++++{liked_message}")
